...
// 게시물 목록을 불러와 렌더링한다.
// 로딩중이면 중복 호출을 막고, 에러 발생시 사용자에게 알린다.
...
async function fetchPosts(){
    ...
    try{ // 에러가 발생할것 같은 코드의 모음
        ...
        } finally{ // 에러 발생 여부와 상관 없이 실행하는 코드
            loading = false;
...
function renderPosts(items){
    ...
    for(const p of items){
        ...
        li.className = 'item';
        li.innerHTML = `<div style="width:36px;height:36px;border-radius:6px;background:#eef8ff;display:flex;align-items:center;justify-content:center;color:var(--accent);font-weight:700">${p.id}</div>
      <div style="flex:1">
